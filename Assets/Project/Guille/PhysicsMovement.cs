	...
	public class PhysicsMovement : MonoBehaviour {
		...
		private void Crouch() {
			...
			if (grounded && !inputJump && Input.GetKey(crouchKey)) {
				currentPlayerHeight = collider.height = playerHeight / 2;
			}
			else {
				currentPlayerHeight = collider.height = playerHeight;
			}
		...
		private void CollisionCheck() {
			...
			Vector3 point0 = transform.TransformPoint(collider.center + new Vector3(0f, currentPlayerHeight/2 - collider.radius, 0f));
			//The center of the sphere at the end of the capsule
			Vector3 point1 = transform.TransformPoint(collider.center - new Vector3(0f, currentPlayerHeight/2 - collider.radius, 0f));
			
			int num = Physics.OverlapCapsuleNonAlloc(point0, point1, collider.radius, overlaps, discludePlayer,
